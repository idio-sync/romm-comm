...
8. **Re-check the gate, seal and store, then revoke the token this one supersedes**, and edit the DM to confirm.

   Re-pairing does **not** replace the previous credential — verified 2026-09-14, a second approval left both tokens live on the account, same device, same name, neither expiring. Left alone, every re-pair abandons a fully-scoped credential that the bot stops tracking the moment it overwrites its row: invisible to the audit view, unreachable by `/unpair`, and permanent. A user who re-pairs *because they think their credential leaked* would be making the problem worse, which is the opposite of what that action should mean.

   So the store, after successfully writing the new row, admin-deletes the superseded token — the `token_id` from the row it just replaced, plus any other token on that `device_id` whose name carries the bot's prefix and which is not the one just minted. Ordering matters: store first, revoke second, so a failed revoke leaves the user with a working pairing rather than none. A revoke that fails is logged and surfaced in `/pairings` for the admin to finish.

   This is also what `/pairings`' reconcile action is for: tokens abandoned before this rule existed, or by a revoke that failed, are exactly the orphans it lists.
...
- **`test_pair_supersede.py`** — re-pairing revokes the token it replaced, and does so *after* the new row is written; a failed revoke leaves the new pairing intact and is surfaced rather than swallowed; the revoke targets only tokens on our `device_id` carrying our name prefix, and never the one just minted. Given tokens demonstrably accumulate, a regression here silently strands a live credential per re-pair.
...
8. **Whether re-pairing with a stable `client_device_identifier` reuses the device row, and whether it replaces or accumulates client tokens.** Determines whether step 6's `created_at` tiebreak is sufficient or whether stale tokens pile up per user.
   ...
   **Answered** (2026-09-14). The device row *is* reused — the same `client_device_identifier` returned the same `device_id` (`7d439947-…`) across three pairings, so step 3's rationale holds. But **client tokens accumulate**: a third approval left `id 2` and `id 3` both live, same `device_id`, same name, both non-expiring.

   So the `created_at` tiebreak is load-bearing, not defensive — and accumulation needs a design response of its own, below.
