...
class IGDBClient:
    ...
    async def search_game(self, game_name: str, platform_name: str = None) -> List[Dict]:
        """Search for a game on IGDB with multiple search strategies"""
        try:
            ...
            # Prepare multiple search attempts (similar to /search command)
            search_attempts = []
            ...
            if len(words) > 1:
                ...
                # Try without common words like "the", "of", etc.
            ...
            search_attempts = unique_attempts[:3]  # Limit to 3 attempts to avoid too many API calls
            ...
            for attempt in search_attempts:
                results = await self._perform_igdb_search(session, headers, attempt, platform_name)
            ...
            all_results = self._sort_by_relevance(all_results, game_name)
    ...
    async def _perform_igdb_search(self, session: aiohttp.ClientSession, headers: dict, 
                                  search_term: str, platform_name: str = None) -> List[Dict]:
        """Perform a single IGDB search"""
        try:
            url = "https://api.igdb.com/v4/games"
            
            # Build the IGDB query - use both search and where clause for better matching
            # IGDB search is already case-insensitive
            query = (
            ...
            if platform_name:
                # Try to match platform name more flexibly
                query += f' where platforms.name ~ *"{platform_name}"*;'
            
            query += " limit 20;"  # Get more results initially for better filtering
    ...
    def _sort_by_relevance(self, games: List[Dict], original_query: str) -> List[Dict]:
